...
class HardBinaryConv(nn.Module):
    ...
    def forward(self, x):
        if HardBinaryConv.LATENT_WEIGHTS:
            latent_weights = self.weight
            scaling_factor = torch.mean(
                torch.mean(torch.mean(abs(latent_weights), dim=3, keepdim=True), dim=2, keepdim=True), dim=1,
                keepdim=True)
            scaling_factor = scaling_factor.detach()
            binary_weights_no_grad = scaling_factor * torch.sign(latent_weights)
            clipped_weights = torch.clamp(latent_weights, -1.0, 1.0)
            binary_weights = binary_weights_no_grad.detach() - clipped_weights.detach() + clipped_weights
        else:
            # If not using latent weights the weight is already the binary weight
